...
# Description:  JK-FF Project Makefile
#################################################################
...
### PROJECT ###
PROJECT             = jk_ff_example
TOP_MODULE          = jk_ff_example
...
create-sdc:
	rm -rf $(PROJECT_SDC)
	@(echo "create_clock -name $(CLOCK_PORT_A) -period $(CLOCK_PERIOD) [get_ports {$(CLOCK_PORT_A)}]" > $(PROJECT_SDC))
	@(echo "create_clock -name $(CLOCK_PORT_B) -period $(CLOCK_PERIOD) [get_ports {$(CLOCK_PORT_B)}]" >> $(PROJECT_SDC))
